 ...
 */	
 class eventos extends OPF_myController {
 	...
 	public function onClickCancelConfirm (){
 		...
 		$this->messageBox(htmlentities(OPF_PROFILES_1),'help',array(htmlentities(YES)=>'onClickCancel',NO));
 	...
 	public function onClickDeleteRecord ($id){
 		...
 		if ($ess_profiles->delete($id)){
 			...
 			$this->notificationWindow(htmlentities(OPF_PROFILES_2),3,'ok');
 	...
 	public function onClickAddRecord ($idProfile){
 		...
 		$this->modalWindow($OPF_profiles->getFormAddModProfile($id),htmlentities(OPF_PROFILES_3),300,250,2);
