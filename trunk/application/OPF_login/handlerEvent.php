 ...
 class eventos extends OPF_myController {
 	...
 	public function onChangeEngine ($engine){
 		...
 		if ($engine){
 			...
 			if (!extension_loaded($ext)){
 				...
 				$this->messageBox('['.$ext.']&nbsp;'.htmlentities(OPF_LOGIN_28),'error');
 	...
 	public function onClickCreateBD ($datForm){
 		...
 		if ($this->MYFORM_validate($datForm, array('engine','encoding','db'))){
 			...
 			switch ($datForm['engine']){
 				...
 				case 'mysql':
 					...
 					if (!$link){
 						...
 						$this->messageBox(htmlentities(OPF_LOGIN_18),'error');
 						...
 						if (@mysql_query($sql, $link)) {
 							...
 							$this->messageBox(htmlentities(OPF_LOGIN_19),'info');
 								
 							$this->assign('content1', 'innerHTML', $OPF_login->getFormInstall('tables',$datForm));
 								
 						} else {
 					
 							$this->messageBox(htmlentities(OPF_LOGIN_20).' '.mysql_error($link),'error');
 				...
 				case 'pgsql':
 					...
 					if (!$link){
 						...
 						$this->messageBox(htmlentities(OPF_LOGIN_23),'error');
 						...
 						if (@pg_query($link, $sql)) {
 							...
 							if (!$link){
 								...
 								$this->messageBox(htmlentities(OPF_LOGIN_20).' '.pg_errormessage(),'error');
 			...
 			$this->messageBox(htmlentities(OPF_LOGIN_22), 'error');
 		}
 			...
 			switch ($datForm['engine']){
 				...
 				case 'mysql':
 					...
 					if (!$link){
 						...
 						$this->messageBox(htmlentities(OPF_LOGIN_6),'error');
 						...
 						if (@mysql_select_db ($datForm['db'], $link)){
 							...
 							$this->messageBox(htmlentities(OPF_LOGIN_21),'error');
 						}
