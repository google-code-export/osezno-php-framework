		...
		while ($nCurrent < $nLength)
		{
				...
				if ($sExpecting == substr($sArg, $nCurrent, $nExpectedLength))
				{
					...
					case 6:	// /v
						if ($nLast < $nCurrent)
						{
							...
							switch ($cType) {
								case 'S': $mValue = substr($mValue, 1); break;
								case 'B': $mValue = (bool)substr($mValue, 1); break;
								case 'N': $mValue = floatval(substr($mValue, 1)); break;
			...
			if (false == $bFound)
			{
				if (0 == $nCurrent)
				{
					...
					switch ($cType) {
					    case 'S': $sArg = substr($sArg, 1); break;
					    case 'B': $sArg = (bool)substr($sArg, 1); break;
					    case 'N': $sArg = floatval(substr($sArg, 1)); break;
